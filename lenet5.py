...
class LeNet5_LERU_MAX(object):
    ...
    def _build_net(self):
        ...
        with tf.name_scope('pool_2'):
            self.pool2 = tfg.max_pool(self.conv2, 2, 1, 'pool2', 'VALID')
            print('pool_2: ', self.pool2.get_shape())
            
        with tf.name_scope('conv_3'):
            self.conv3 = tfg.conv2d(self.pool2, 5, 1, 48, 'conv3', 'VALID')
            print('conv_3: ', self.conv3.get_shape())
            
        with tf.name_scope('pool_3'):
            self.pool3 = tfg.max_pool(self.conv3, 2, 1, 'pool3', 'VALID')
