...
class LeNet5(object):
    ...
    def _build_net(self):
        ...
        """
        with tf.name_scope('fc_1'):
            self.fc1 = tfg.conv2d(self.pool2, 5, 1, 120, 'fc_1', 'VALID','SIGMOID')            
            print('fc_1: ', self.fc1.get_shape())
            
        with tf.name_scope('flat_1'):
            self.flat1, self.flat_dim = tfg.flatten(self.fc1)
            print('flat_1:', self.flat1.get_shape())
            
        with tf.name_scope('fc_2'):
            self.fc2 = tfg.fc_layer(self.flat1, self.flat_dim, 84, 'fc2')
